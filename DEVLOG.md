...
- **改动者：** hzx
- **类型：** 文档更新
- **改动文件：** `引脚汇总.md`, `DEVLOG.md`
- **内容：**
  - 新建 `引脚汇总.md`，记录 STM32 实际硬件引脚映射表（PWM/方向/编码器/其他）
  - 编码器 E4 (D电机) 实际使用 PA8 (A相) / PA4 (B相)，与代码中 PB10/PB11 不同，代码待适配
  - C 电机编码器 E3 实际接线 PA7 (A相) / PA6 (B相)，与代码中 PA6 (A相) / PA7 (B相) 的 A/B 相对应交换
  - B 电机方向引脚实际接线 BIN1=PB13 / BIN2=PB12，与 DEVLOG 中标注的 PB12 (BIN1) / PB13 (BIN2) 相反
  - STBY 引脚直连 3.3V（常使能）
- **待办：** 代码需根据实际引脚映射进行适配调整
=======
### 2026-06-05 — D编码器改用TIM6中断轮询 + 频率提升至50kHz

- **改动者：** tulip627722
- **类型：** Bug修复 / 参数调整
- **改动文件：** HAREWER/ENCODER/encoder.c, HAREWER/ENCODER/encoder.h, USER/main.c
- **内容：**
  - D编码器原用主循环50ms轮询，高速时脉冲被全部漏掉，读数一直为0/-1
  - 改为TIM6定时器中断轮询（先1kHz，后10kHz，最终50kHz）
  - 50kHz下D轮读数从300提升至3000+，与其他轮（5000+）接近
  - TIM6中断优先级设为1，不影响其他外设中断
  - PB8/PB4引脚不受TIM2完全重映射干扰
- **验证：** D编码器可稳定读数，四轮速度闭环控制正常

>>>>>>> c70f938 (docs: 更新开发日志 - D编码器TIM6 50kHz轮询方案)
