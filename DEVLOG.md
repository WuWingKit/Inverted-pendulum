...
### 2026-06-17 — 修复小角度漂移正反馈路径

- **改动者：** WuWingKit
- **类型：** Bug 修复 / 控制策略调整
- **改动文件：** `HAREWER/BALANCE/balance.c`, `HAREWER/BALANCE/balance.h`, `USER/main.c`, `DEVLOG.md`
- **内容：**
  - 通过多代理代码审查确认，小角度越调越偏的关键风险不是单个 PID 参数，而是运行中零偏学习、hold 区清位置、漂移限幅仍同向输出等机制叠加，容易让小角度误差被吃掉并形成单方向速度累积。
  - 新增 `BALANCE_AUTO_BIAS_ENABLE = 0`，默认禁用运行中的自动角度 Bias 学习，避免把真实小角度倾斜逐步学成零点；零点仍通过 KEY2/串口调零设置。
  - 修改 hold 区逻辑：只有未漂移时才允许极小角度静止输出 0，并且不再清零 `Balance_Position`，避免慢速漂移时位置回中信息被反复抹掉。
  - 修改漂移回拉逻辑：当小车已经漂移且角度仍小于 `BALANCE_DRIFT_RECENTER_ANGLE_X100` 时，如果当前输出仍会把车推得更远，则强制给 `BALANCE_DRIFT_PULL_PWM` 的反向回拉输出，而不是仅限幅同方向输出。
  - 串口输出增加 `Pos` 字段，方便观察 `Balance_Position` 是否还在单方向累积，验证漂移回拉是否真实生效。
- **验证：** 待重新编译烧录。重点观察小角度时 `Pos` 是否不再被清零且能够被拉回；若 `SF/Pos` 仍与 PWM 同向加速，需要继续排查编码器速度符号。

### 2026-06-17 — 下调电机 PWM 死区补偿

- **改动者：** WuWingKit
- **类型：** 参数调整 / 电机输出补偿
- **改动文件：** `HAREWER/PWM/pwm.c`, `DEVLOG.md`
- **内容：**
  - 根据实车调试需求，将 `MOTOR_PWM_DEADZONE` 从 `1100` 下调到 `900`。
  - 保留 `MOTOR_PWM_CMD_FLOOR = 120` 和 `MOTOR_PWM_DEADZONE_RAMP = 900` 的渐进补偿逻辑，小命令仍会从 0 平滑过渡到完整死区补偿。
  - 目标是降低小角度附近和脉冲恢复阶段的输出突兀感，减少电机补偿过强导致的抖动或过冲。
- **验证：** 待重新编译烧录。重点观察小角度微调是否更柔和；若电机又出现“想动但不动”，可回调到 `1000`。

### 2026-06-17 — 增加漂移急加速脉冲

- **改动者：** WuWingKit
- **类型：** 控制策略调整 / 防倒策略
- **改动文件：** `HAREWER/BALANCE/balance.c`, `HAREWER/BALANCE/balance.h`, `DEVLOG.md`
- **内容：**
  - 根据实车反馈“如果往一个方向一直偏移，希望突然增加一个急加速把杆子晃到另一边，然后再慢一点减速把速度控回来”，新增漂移脉冲状态机。
  - 当小车处于漂移状态、摆杆继续远离零点、角度超过约 `1.8°` 且车速已明显累积时，触发 `BALANCE_KICK_PWM = 5200` 的短促急加速脉冲。
  - 脉冲持续 `BALANCE_KICK_TICKS = 18` 个控制周期，当前 `2ms` 周期下约 `36ms`，用于快速给杆子一个足够大的反向摆动趋势。
  - 脉冲结束后进入 `BALANCE_RECOVER_TICKS = 90` 的慢恢复阶段，使用 `BALANCE_RECOVER_SPEED_KP = 0.70`，避免立即猛刹导致杆子又被减速加速度拉回同一侧。
  - 新增 `BALANCE_KICK_COOLDOWN_TICKS = 120` 冷却时间，避免连续脉冲造成抽动；漂移限幅不作用于脉冲期间，保证急加速真实生效。
- **验证：** 待重新编译烧录。重点观察单方向漂移时是否会出现短促急加速，并在之后较慢减速；若脉冲过猛，可降低 `BALANCE_KICK_PWM` 或 `BALANCE_KICK_TICKS`。
...
- **改动文件：** `HAREWER/BALANCE/balance.c`, `HAREWER/BALANCE/balance.h`, `DEVLOG.md`
- **内容：**
  - 根据实车反馈“中间晃住稳定的时间变久了，但仍会晃着晃着往某个方向跑”，在防倒优先策略上增加更明确的漂移回拉。
  - 新增 `Balance_Drift_Position_Kp = 0.012`，漂移状态下使用更强的位置回拉；正常状态仍使用 `Balance_Position_Kp = 0.005`，避免过度影响中间晃动。
  - 将 `BALANCE_DRIFT_POSITION` 从 `12000` 降到 `7000`，更早识别小车已经开始离开中心区域。
  - 新增 `BALANCE_DRIFT_PUSH_LIMIT = 900`，漂移状态下如果输出方向仍然会把小车推向更远位置，则限制该方向输出；反向回拉输出不限制。
  - 目标是在允许杆子持续摆动的前提下，避免小车越晃越远，保留速度余量用于下一次救杆。
- **验证：** 待重新编译烧录。重点观察小车是否会在跑远后主动往回拉，而不是只减慢单方向漂移。
...
- **改动文件：** `HAREWER/BALANCE/balance.c`, `HAREWER/BALANCE/balance.h`, `DEVLOG.md`
- **内容：**
  - 根据实车反馈“仍然会往一个方向跑，希望小车尽量保持不动；大角度策略是对的，不要区分大小角度，而是一直把角度一点点调小”，简化平衡控制逻辑。
  - 删除小角度软控制、回正阶段 Carry 弱阻尼、中心前防反向刹车等分段控制路径，避免不同角度区间切换时出现“中间停住/单方向加速”的行为。
  - 全程统一使用 `Balance_Angle_Kp = 4.2` 和 `Balance_Angle_Kd = 48.0` 做角度 PD，让角度误差连续被压小，不再因为进入小角度区而切换到另一套策略。
  - 将救杆补偿阈值 `BALANCE_RESCUE_ANGLE_X100` 从 `500` 降到 `120`，约 `1.2°` 后即开始连续叠加救杆补偿，使大角度策略从小角度就逐步生效。
  - 速度和位置反馈改为持续参与，`position_pwm` 不再因 falling/returning 阶段被暂停，目标是让小车尽量保持在原地附近，而不是长期往单方向跑。
  - 保留极小角度静止窗口、速度漂移保护和最小输出爬坡；这些只用于防抖和防跑偏，不再改变角度控制策略。
- **验证：** 待重新编译烧录。重点观察小角度阶段是否不再出现“到中间停住然后单方向加速”，以及小车是否更倾向于围绕起点附近修正。
