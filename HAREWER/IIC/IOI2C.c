...
void IIC_Init(void)
{
	...
	GPIOB->CRH&=0XFF00FFFF;//PB10/11 推挽输出
...
*功能介绍:	    //读1个字节，ack=1时，发送ACK，ack=0，发送nACK
*******************************************************************************/
...
*功能介绍:	    将 修改 写 指定设备 指定寄存器一个字节 中的多位
参数	dev  目标设备地址
		reg	    寄存器地址
		bitStart  目标字节的起始位
		length   位数
...
*功能介绍:	    将 修改 写 指定设备 指定寄存器一个字节 中的1个位
参数	dev  目标设备地址
		reg	    寄存器地址
		bitNum  要修改的目标字节的bitNum位
		data  为0 时目标位将置0 否则将置位
返回   成功 为1
 		失败为0
*******************************************************************************/
